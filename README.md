...
WhatsApp delivery is optional and uses Twilio. It tries to send the **full forecast as a PNG image** (the same image attached to the email) so the monospace columns stay aligned on a phone. Twilio fetches WhatsApp media server-side, so the image is uploaded to a public host (uguu.se, falling back to catbox.moe) and the resulting URL is passed to Twilio. If the upload fails, it falls back to sending the forecast as text, split into messages if it exceeds WhatsApp's 4096-character limit.

> **Twilio trial limitation:** trial accounts reject the media parameter with "Invalid or disallowed parameters provided - trial accounts have limited parameter access". When that happens the code automatically retries as a plain text message, so you still get the forecast. Upgrading the Twilio account unlocks image delivery.
